...
- `install-qq` 默认下载 NapCat 兼容包 `linuxqq 3.2.23-44343`（第三方备份源；官方 dldir1 链已 404）。
- 可用 `NAPCAT_QQ_PACKAGE_URL` 覆盖包地址；`napcat install-qq --force` 强制重装/降级。
- 注意：过新的 LinuxQQ（如 `3.2.31-51102`）可能不在 PacketBackend 支持表内，表现为扫码可登录、重启后快速登录失效。
